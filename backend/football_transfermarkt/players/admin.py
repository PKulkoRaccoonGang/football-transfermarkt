...
class PlayerAdmin(admin.ModelAdmin):
    list_display = (
        'first_name', 'last_name', 'team', 'position', 'jersey_number',
        'goals_scored', 'assists_per_match', 'is_active', 'get_photo_preview'
    )
    ...
    def team_photo_preview(self, obj):
        if obj.team and obj.team.photo:
            return mark_safe(f'<img src="{obj.team.photo.url}" style="max-height: 200px; max-width: 200px;" />')
        return "No team photo available"
    team_photo_preview.short_description = "Team Photo Preview"

    def get_photo_preview(self, obj):
        if obj.photo:
            return mark_safe(f'<img src="{obj.photo.url}" style="max-height: 50px; max-width: 50px;" />')
        return "No photo"
...
class TeamAdmin(admin.ModelAdmin):
    list_display = ('name', 'city', 'country', 'foundation_year', 'stadium', 'coach', 'players_count', 'get_photo_preview')
    ...
    def players_list(self, obj):
        ...
        for player in players:
            ...
            player_link = f'<a href="/admin/players/player/{player.id}/change/" target="_blank">{player.first_name} {player.last_name}</a>'
            player_info = f"{player_photo} {player_link} (#{player.jersey_number})"
